...
export default async function ExpenditurePage({ searchParams }: Props) {
  ...
  const result = await api.fiscal.expenditure(filters);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-slate-500">
            Revenue and capital expenditure across major budget heads
          </p>
      ...
      {result && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <MetricCard
            ...
            description="Expenditure observations matching filters"
            basis={basis}
            lastUpdated={now}
          />
        </div>
